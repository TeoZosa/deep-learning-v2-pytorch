...
def compare_init_weights(
    model_list, plot_title, train_loader, valid_loader, plot_n_batches=100
):
    """
    ...
    for label, val_acc in label_accs:
        print(f"  {val_acc * 100:7.3f}% -- {label}")
    print("Training Loss")
    for label, loss in label_loss:
        print(f"  {loss:7.3f}  -- {label}")
...
def hist_dist(title, distribution_tensor, hist_range=(-4, 4)):
    ...
    plt.title(title)
    plt.hist(
        distribution_tensor, np.linspace(*hist_range, num=len(distribution_tensor) // 2)
    )
    plt.show()
